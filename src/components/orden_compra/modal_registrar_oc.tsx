...
type OCDialogWrapperProps = {
  visible: boolean;
  onHide: () => void;
  onCreated: (orden: any) => void; // 👈 callback para avisar al padre
};

export default function OCDialogWrapper({
  visible,
  onHide,
  onCreated,
}: OCDialogWrapperProps) {
...
const router = useRouter();
  ...
  const handleGenerarOrden = async () => {
    ...
    if (!fecha) {
      alert("Selecciona una fecha de entrega estimada");
      return;
    }
    ...
    const hoy = new Date();
    if (fecha <= hoy) {
      alert("La fecha de entrega estimada debe ser posterior a hoy");
      return;
    }
    ...
    try {
      ...
      // 🔥 avisar al padre que se creó la orden
      onCreated(data);
    ...
    >
      ...
      <div className="mt-4">
        <TableInsumos
          ...
          onDelete={(row) =>
            setInsumosAgregados(
              insumosAgregados.filter(
                (i) => i.nombre_insumo !== row.nombre_insumo
              )
            )
          }
        />
